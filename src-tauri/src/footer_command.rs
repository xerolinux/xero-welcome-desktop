...
#[tauri::command]
pub async fn open_xero_kofi() {
    let _output = Command::new("firefox").arg("https://ko-fi.com/xerolinux").output().expect("[!] open_xero_patreon -> Command executed with failing error code");
}
#[tauri::command]
...
#[tauri::command]
pub async fn open_xero_liberapay() {
    let _output = Command::new("firefox").arg("https://liberapay.com/DarkXero/").output().expect("[!] open_xero_fundrazr -> Command executed with failing error code");
}
