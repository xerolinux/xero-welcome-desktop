...
import apps from "../data/app_util/with-desc.json";
...
export default function AppsList() {
  ...
  return (
    <>
    <div className="row" style={{ marginTop: -65, marginBottom: 40, marginRight: "-60%" }}>
        <button
          type="button"
          class="text-white bg-gradient-to-b from-green-900 to-green-600 hover:bg-gradient-to-r focus:ring-4 focus:outline-none focus:ring-purple-200 dark:focus:ring-purple-800 font-medium rounded-lg text-sm py-2.5 text-center flex"
...
function InnerTable(props) {
  /*
  // TODO: Save currently selected apps
  const saveValues = () => {
    let uniqueObject = {};
    for (let i in appInstallList) {
      // Extract the name
      let objName = appInstallList[i]["name"];

      // Use the name as the index
      uniqueObject[objName] = appInstallList[i];
    }
    for (let i in uniqueObject) {
      saveList.push(uniqueObject[i]);
    }
    localStorage.setItem("installed_apps", JSON.stringify(saveList));
  };

  // TODO: Load currently installed apps
  const loadValues = () => {
    let i = localStorage.getItem("installed_apps");

    if (i === null) {
      setAppInstallList([]);
    } else {
      setAppInstallList(JSON.parse(i));
    }
  };
  */
  ...
  const removeApp = (pkg) => {
    for (var i = 0; i < appInstallList.length; i++) {
      if (appInstallList[i] === pkg) {
        appInstallList.splice(i, 1);
      }
    }
  };
  ...
  function addApp(pkg) {
    if (appInstallList.length === 0) {
      appInstallList.push(pkg);
    } else {
      appInstallList.indexOf(pkg) === -1 ? appInstallList.push(pkg) : console.log("This item already exists");
    }
  }
  ...
  return (
    <>
      {app.apps.map((a) => {
        let imgRef = useRef();
        let onImageError = () =>
          (imgRef.current.src = "./app-icons/settings.svg"); // Fallback icon in case one is not found
